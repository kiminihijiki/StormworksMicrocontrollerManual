...
Lua本文だけなら、パーツカタログから論理ノードを調べてI/O契約を作ります。マイコン一式なら、ゲートやLuaノードの構造は外部のDSL/XML検証環境が提供する `spec` を使います。

## 2. 参照DSLを先に調べる

外部のDSL/XML検証環境を作業環境に用意した場合は、まず次を実行します。
...
`script_ref` を使う場合のファイルパスは、外部のDSL/XML検証環境の仕様に従い、`script_ref` を含む `.sw-net` 文書からの相対パスとして管理します。最終的には次の検証を行います。
...
- `verified`：外部資料の検証済みメモ、または明示的なゲーム内確認に基づく。
